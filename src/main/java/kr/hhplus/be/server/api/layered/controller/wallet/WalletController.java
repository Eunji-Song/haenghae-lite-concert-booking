package kr.hhplus.be.server.api.layered.controller.wallet;
...
public class WalletController {
    ...
    @PostMapping("/charge")
    public ResponseEntity<WalletBalanceResponse> charge(@CurrentUserUuid String userUuid,
                                                       @RequestHeader("Idempotency-Key") String idemKey,
                                                       @RequestBody @Valid WalletChargeRequest request) {
        return ResponseEntity.ok(walletService.charge(userUuid, request, idemKey));
    }
}